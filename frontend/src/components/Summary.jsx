function Summary({ tables, pedidos, updatePedidos, updateTableStatus }) {
        ...
        {pedidos.length > 0 ? (
          pedidos.map((pedido) => (
            <div key={pedido.id} className="card mb-2">
              ...
              <button onClick={()=>{
                addPedido(pedido.id,pedido.mesa_id,pedido.customer_name,pedido.notes)
                updatePedidos();
                updateTableStatus(pedido.mesa_id,tablaDefualt.status,tablaDefualt.customerName,tablaDefualt.notas)
